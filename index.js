...
const coverLetterText = require("./cover");
...
(async () => {
  try {
    ...
    await page.waitForSelector(
      'button[data-toggle="modal"][data-target="#login-modal"].login-cta',
      { visible: true }
    );
    await page.click(
      'button[data-toggle="modal"][data-target="#login-modal"].login-cta'
    ...
    await page.type("#modal_email", "put_your_email_here", {
      delay: 100,
    });
    await page.waitForSelector("#modal_password", { visible: true });
    await page.type(
      "#modal_password",
      "put_your_passwordFor_internshala_here",
      { delay: 100 }
    );
    ...
    await page.waitForSelector("a.nav-link.dropdown-toggle.internship_link", {
      visible: true,
    });
    ...
    if (chooseToWorkFromHome) {
      ...
      console.log(
        isChecked
          ? "Work from Home checkbox is now checked."
          : "Failed to check Work from Home checkbox."
      );
    }
    ...
    if (choosePartTimeWork) {
      ...
      console.log(
        isChecked
          ? "Part-time checkbox is now checked."
          : "Failed to check Part-time checkbox."
      );
    }

    // Search for internships
    await page.waitForSelector('input[value="e.g. Marketing"]', {
      visible: true,
    });
    await page.type('input[value="e.g. Marketing"]', profileName, {
      delay: 100,
    });
    await page.keyboard.press("Enter");
    console.log(
      `Typed "${profileName}" into profile search input & pressed Enter.`
    );
    await new Promise((resolve) => setTimeout(resolve, 5000));

    if (locationName) {
      await page.waitForSelector('input[value="e.g. Delhi"]', {
        visible: true,
      });
      await page.type('input[value="e.g. Delhi"]', locationName, {
        delay: 100,
      });
      console.log(
        `Typed "${locationName}" into location search input and pressed Enter.`
      );
      await page.keyboard.press("Enter");
    }
    await new Promise((resolve) => setTimeout(resolve, 5000));
    ...
    const internshipContainers = await page.$$(
      "div.individual_internship_details.individual_internship_internship"
    );
    ...
    if (internshipContainers.length === 0) {
      ...
      for (let index = 1; index < 5; index++) {
        await internshipContainers[index].click();
        console.log("Clicked on the first internship heading container.");
        await new Promise((resolve) => setTimeout(resolve, 10000));

        try {
          await page.waitForSelector("button#continue_button.btn.btn-large", {
            visible: true,
            timeout: 60000,
          });
        ...
        const countAndLogAssessmentQuestions = async () => {
          ...
          console.log(
            `Number of assessment questions: ${questions.length - 1}`
        ...
        if (textareas.length === 0) {
          ...
          for (let i = 1; i < textareas.length; i++) {
            ...
            console.log(`Typed answer into textarea ${i + 1}`);
